...
int main(int argc, char** argv)
{
	...
	DT * mem_t0, *mem_t1, *mem_t2, *mem_t3;
	...
	{
		mem_t0 = (DT *)_mm_malloc(bytes_max, 64);
		mem_t1 = (DT *)_mm_malloc(bytes_max, 64);
		mem_t2 = (DT *)_mm_malloc(bytes_max, 64);
		mem_t3 = (DT *)_mm_malloc(bytes_max, 64);
		...
		fill(mem_t0, elems, 1.0);
		fill(mem_t1, elems, 2.0);
		fill(mem_t2, elems, 3.0);
		fill(mem_t3, elems, 4.0);
		...
		{			
			if(p==(samples-1))
				t_start = timer();	
			#pragma omp parallel shared(ret_t0, ret_t1, mem_t0, mem_t1, reps, elems) \
								private(tid)
			{
				...
				if(tid==3)
				{
					for(r=0; r<reps; r++)
					{
						...
						{
							//ret_t3 += mem_t3[i];
							memO_t3[i] = alpha * mem_t3[i];
						}				
					 memO_t3[0] = memO_t3[0] * 0.1; // to avoid overflow and optimizations
					}								
				}				
		...
		if(mem_t2!=NULL) _mm_free(mem_t2);
		if(mem_t3!=NULL) _mm_free(mem_t3);
